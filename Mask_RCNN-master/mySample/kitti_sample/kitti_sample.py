...
'''
这段代码是一个小的测试代码，主要是用 vgi自己用kitti数据集的照片制作 label 然后 改动部分的bollon.py代码 测试用 -by 杨彬
'''
import os
import sys
import json
...
class KITTIDataset(utils.Dataset):
    ...
    def load_kitti(self, dataset_dir, subset):
        ...
        annotations = json.load(open(os.path.join(dataset_dir, "via_region_data.json")))  #这里我把jason文件名字改了
        #根据文件结构适当做出改变
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        for i, p in enumerate(info["polygons"]):
            ...
            if 'all_points_y' in p.keys() and 'all_points_x' in p.keys():
